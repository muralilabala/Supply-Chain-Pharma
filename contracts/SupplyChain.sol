...
contract SupplyChain {
    ...
    function addRMS(
        ...
        if(rmsCtr>0){
            require(findRMS(_address) == 0); //Only one raw material supplier can be added
        }
        //require(findRMS(_address) == 0); //Only one raw material supplier can be added
        rmsCtr++;
    ...
    function addManufacturer(
        ...
        if(manCtr>0){
            require(findMAN(_address) == 0); //Only one raw material supplier can be added
        }
        //require(findMAN(_address) == 0); //Only one manufacturer can be added
        manCtr++;
    ...
    function addDistributor(
        ...
        if(disCtr>0){
            require(findDIS(_address) == 0); //Only one raw material supplier can be added
        }
        //require(findDIS(_address) == 0); //Only one distributor can be added
        disCtr++;
    ...
    function addRetailer(
        ...
        if(retCtr>0){
            require(findRET(_address) == 0); //Only one raw material supplier can be added
        }
        //require(findRET(_address) == 0); //Only one retailer can be added
        retCtr++;
